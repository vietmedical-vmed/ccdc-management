...
(function () {
  ...
  function AddWizard({ fy, onDone, onCancel }) {
      ...
      api("ngan_sach_items", {
        ...
        for (const row of (r.rows || [])) {
          if (row.existing_sl != null)
            pre[row.ma_bravo] = { checked: true, sl: row.existing_sl, id: row.existing_id, don_gia: row.existing_don_gia ?? (row.don_gia_mua != null ? Number(row.don_gia_mua) : null) };
        }
    ...
    const toggle = (ma) => {
      setSelected(prev => {
        ...
        const defaultPrice = item?.don_gia_mua != null ? Number(item.don_gia_mua) : null;
        return { ...prev, [ma]: { checked: true, sl: cur?.sl || 1, id: cur?.id || null, don_gia: cur?.don_gia ?? defaultPrice } };
      });
    };
    ...
    const setDonGia = (ma, val) => {
      setSelected(prev => ({
        ...prev,
        [ma]: { ...prev[ma], don_gia: val === "" ? null : Number(val) || 0 },
      }));
    };
      ...
      if (allChecked) {
        ...
        setSelected(prev => {
          ...
          for (const m of visible) {
            if (!next[m]?.checked) {
              const item = items.find(r => r.ma_bravo === m);
              const dp = item?.don_gia_mua != null ? Number(item.don_gia_mua) : null;
              next[m] = { checked: true, sl: 1, id: null, don_gia: dp };
            }
          }
          return next;
        });
    ...
    const doSave = async () => {
      ...
      try {
        const res = await api("batch_upsert_ngan_sach", {
          nam_tai_chinh: fy,
          mien: mien || null,
          items: checkedItems.map(r => ({ ma_bravo: r.ma_bravo, so_luong: r.so_luong, don_gia: r.don_gia, id: r.id })),
        });
            ...
            h("tbody", null,
              filteredItems.length === 0 && !loadingItems
                ...
                : filteredItems.map(r => {
                    ...
                    const donGia = checked ? sel.don_gia : (r.don_gia_mua != null ? Number(r.don_gia_mua) : null);
                    ...
                    return h("tr", {
                      ...
                      h("td", { className: cellCls + " text-right" },
                        checked
                          ? h("input", {
                              type: "number", min: 0,
                              className: "w-24 px-2 py-1 text-sm text-right tabular-nums border border-slate-300 rounded outline-none focus:border-blue-400",
                              value: sel.don_gia ?? "",
                              placeholder: "—",
                              onChange: e => setDonGia(r.ma_bravo, e.target.value),
                            })
                          : h("span", { className: "tabular-nums text-slate-600" }, donGia != null ? fmtMoney(donGia) : "—"),
                      ),
                      h("td", { className: cellCls + " text-right" },
                        checked
                          ? h("input", {
  ...
  function UploadExcel({ fy, onDone, onCancel }) {
    ...
    const HEADER_MAP = {
      ...
      "đơn giá": "don_gia", "don gia": "don_gia", "đơn giá mua": "don_gia", "don gia mua": "don_gia", "giá": "don_gia",
      ...
      reader.onload = (e) => {
        try {
          ...
          for (let i = 1; i < raw.length; i++) {
            ...
            if (!ma || sl <= 0) continue;
            const dg = colMap.don_gia != null ? Number(r[colMap.don_gia]) : null;
            rows.push({
      ...
      try {
        ...
        for (const [mien, items] of Object.entries(byMien)) {
          const res = await api("batch_upsert_ngan_sach", {
            nam_tai_chinh: fy,
            mien: mien || null,
            items: items.map(r => ({ ma_bravo: r.ma_bravo, so_luong: r.so_luong, don_gia: r.don_gia })),
          });
  ...
  function NganSachScreen({ user }) {
    ...
    const startEdit = (r) => setEditing({
      ...
      don_gia: r.don_gia != null ? Number(r.don_gia) : null,
    });
          ...
          h("table", { className: "w-full text-sm" },
            h("thead", { className: "bg-slate-50 border-b border-slate-200" },
              h("tr", null,
                ...
                ["Miền", "Mã NCC", "Mã Bravo", "Tên hàng hóa", "Đơn giá", "SL đăng ký", ""].map((c, i) =>
                  h("th", {
                    key: i,
                    className: "px-3 py-2 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide" +
                      (["Đơn giá", "SL đăng ký"].includes(c) ? " text-right" : ""),
                  }, c)),
              ),
            ),
            h("tbody", null,
              filtered.length === 0 && !loading
                ? h("tr", null, h("td", { colSpan: canWrite ? 8 : 7, className: "px-3 py-8 text-center text-sm text-slate-400" },
                    "Chưa có đăng ký ngân sách — bấm '+ Thêm đăng ký'"))
                : grouped.map(([nhom, items]) => [
                    h("tr", { key: "g-" + nhom, className: "bg-slate-50/70" },
                      h("td", { colSpan: canWrite ? 8 : 7, className: "px-3 py-2 text-xs font-bold text-slate-700 uppercase tracking-wide" },
                        nhom,
                        ...
                        h("span", { className: "ml-2 font-normal normal-case text-slate-500" },
                          (() => {
                            const tv = items.reduce((s, r) => {
                              const p = r.don_gia != null ? Number(r.don_gia) : null;
                    ...
                    ...items.map(r =>
                      editing && editing.id === r.id
                        ? h("tr", { key: r.id, className: "bg-blue-50 border-b border-slate-100" },
                            ...
                            h("td", { className: cellCls + " text-right" },
                              h("input", {
                                type: "number", min: 0, className: cellInput,
                                value: editing.don_gia ?? "",
                                placeholder: "—",
                                onChange: e => setEditing({ ...editing, don_gia: e.target.value === "" ? null : Number(e.target.value) || 0 }),
                              }),
                            ),
                            h("td", { className: cellCls + " text-right" },
                              h("input", {
